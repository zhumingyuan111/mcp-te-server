...
export class AuthFlow {
  ...
  async authenticate(): Promise<string> {
    ...
    log('Browser login successful, obtaining bearerToken...');
    await this.tokenManager.setBearerToken(bearerToken);
    return bearerToken;
  }

  /**
   * Re-authenticate: clear old token and obtain a new one.
   */
  async reauthenticate(): Promise<string> {
    log('Re-authenticating...');

    const newBearerToken = await this.browserLogin();
    await this.tokenManager.setBearerToken(newBearerToken);
